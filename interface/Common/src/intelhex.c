    ...
    while (hex_ptr < hex_end) {
        ...
        if (':' == ch) {
            ...
            if (DATA_RECORD == intelhex_line.record_type) {
                ...
                if (bin_size && ((intelhex_base_address + intelhex_line.address) != *address)) {
                    LOG("Memory from 0x%X to 0x%X is skipped.\n", *address, intelhex_base_address + intelhex_line.address);
                ...
                *address = intelhex_base_address + intelhex_line.address + intelhex_line.byte_count;

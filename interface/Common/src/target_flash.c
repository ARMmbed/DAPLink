...
uint8_t validate_bin_nvic(uint8_t *buf)
{
    ...
    memcpy(&nvic_val, buf+4, sizeof(nvic_val));
    if (0 == test_range(nvic_val, target_device.flash_start, target_device.flash_end)) {
        return 0;
    }
    // test for NMI_Handler
    memcpy(&nvic_val, buf+8, sizeof(nvic_val));
    if (0 == test_range(nvic_val, target_device.flash_start, target_device.flash_end)) {
        return 0;
    }
    // HardFault_Handler
    memcpy(&nvic_val, buf+12, sizeof(nvic_val));
    if (0 == test_range(nvic_val, target_device.flash_start, target_device.flash_end)) {
        return 0;
    }
...
//static uint8_t const zero_buffer[512] = {0};

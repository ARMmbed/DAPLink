...
#define CR_WRITE0_BIT_MASK                     ((uint32_t)0xFFFFFF7F)
...
#define CR_FLOW_CTRL_MASK                      ((uint32_t)0x00000F00)
#define CR_MODE_MASK                           ((uint32_t)0x0000CC00)
...
void FUART_SetIrDAEncodeMode(TSB_FUART_TypeDef * FUARTx, uint32_t Mode)
{
    ...
    tmp &= CR_WRITE0_BIT_MASK;  /* UARTCR bit 7 write as zero */
    FUARTx->CR = tmp;
}
                      ...
                      FunctionalState NewState)
{
    ...
    if (Direction == FUART_TX) {        /* Cofigure Transmit FIFO DMA */
    ...
    } else {                    /* Cofigure Receive FIFO DMA */
...
FUART_AllModemStatus FUART_GetModemStatus(TSB_FUART_TypeDef * FUARTx)
{
    ...
    tmpfr = FUARTx->FR & FR_MODEM_STATUS_MASK;  /* Get RI, DCD, DSR status */
...
#endif                          /* defined(__TMPM366_FUART_H) */

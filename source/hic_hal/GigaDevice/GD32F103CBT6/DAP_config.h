...
{        
    ...
    //        11 output speed_50MHz               11 reserved
    if(pin_bit >= 8) {
        // Clean MODE and CMF
        GPIOx->CTLR2 &= ~(0x0000000F << ((pin_bit-8) << 2));
        // Set MODE 11, ignore CMF
        GPIOx->CTLR2 |= (((uint32_t)(0x00|0x03) & 0x0F) << ((pin_bit-8) << 2));
    }
    else {
        GPIOx->CTLR1 &= ~(0x0000000F << ((pin_bit) << 2));
        GPIOx->CTLR1 |= (((uint32_t)(0x00|0x03) & 0x0F) << ((pin_bit) << 2));
    }
}

static __inline void pin_in(GPIO_TypeDef* GPIOx, uint8_t pin_bit, uint8_t mode)
{
    // mode : 0     analog input
    //        1     float input
    //        2     pullup input 
    ...
    if(pin_bit >= 8) {
        ...
        GPIOx->CTLR1 &= ~(0x0000000F << ((pin_bit) << 2));
        GPIOx->CTLR1 |= ( ((uint32_t)(config) & 0x0F) << ((pin_bit) << 2) );
    }
    if(mode == 2)
        GPIOx->BOR = (((uint32_t)0x01) << pin_bit);
    else if(mode == 3)
        GPIOx->BCR = (((uint32_t)0x01) << pin_bit);
}
...
{
    pin_in(SWCLK_TCK_PIN_PORT, SWCLK_TCK_PIN_Bit, 0);
    pin_in(SWDIO_OUT_PIN_PORT, SWDIO_OUT_PIN_Bit, 0);
    pin_in(SWDIO_IN_PIN_PORT, SWDIO_IN_PIN_Bit, 0);
}
...
{
    return ((SWCLK_TCK_PIN_PORT->DOR & SWCLK_TCK_PIN) ? 1 : 0);
}
...
{
    return ((SWDIO_IN_PIN_PORT->DIR & SWDIO_IN_PIN) ? 1 : 0);
}
...
{
    return ((nRESET_PIN_PORT->DIR >> nRESET_PIN_Bit) & 1);
}
...
static __forceinline void     PIN_nRESET_OUT(uint32_t bit)
{
    if (bit & 1)
        ...
        nRESET_PIN_PORT->BCR = nRESET_PIN;
}
...
{
    if (bit & 1)
        CONNECTED_LED_PORT->BCR = CONNECTED_LED_PIN; // LED on
    else
        CONNECTED_LED_PORT->BOR = CONNECTED_LED_PIN;// LED off
}

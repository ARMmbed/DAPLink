...
void Clk_Brea1_Start(void) 
{
    ...
    Clk_Brea1_CLKEN |= Clk_Brea1_CLKEN_MASK;
	Clk_Brea1_CLKSTBY |= Clk_Brea1_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clk_Brea1_Stop
********************************************************************************
*
* Summary:
...
void Clk_Brea1_Stop(void) 
{
    ...
    Clk_Brea1_CLKEN &= (uint8)(~Clk_Brea1_CLKEN_MASK);
	Clk_Brea1_CLKSTBY &= (uint8)(~Clk_Brea1_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clk_Brea1_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Clk_Brea1_CLKEN & Clk_Brea1_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Clk_Brea1_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clk_Brea1_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clk_Brea1__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clk_Brea1_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clk_Brea1_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clk_Brea1_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Clk_Brea1_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Clk_Brea1_GetSourceRegister();
    uint16 oldDivider = Clk_Brea1_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clk_Brea1_CLKEN & Clk_Brea1_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clk_Brea1_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Clk_Brea1_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Clk_Brea1_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clk_Brea1_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Clk_Brea1_MOD_SRC |= modeBitMask & (uint8)Clk_Brea1_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clk_Brea1_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Clk_Brea1_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clk_Brea1_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clk_Brea1_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Clk_Brea1_MOD_SRC & (uint8)(Clk_Brea1_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clk_Brea1_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Clk_Brea1_GetDividerRegister();
    uint8 oldSrc = Clk_Brea1_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clk_Brea1_MOD_SRC |= CYCLK_SSS;
        Clk_Brea1_MOD_SRC =
            (Clk_Brea1_MOD_SRC & (uint8)(~Clk_Brea1_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clk_Brea1_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Clk_Brea1_MOD_SRC & Clk_Brea1_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clk_Brea1_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Clk_Brea1_PHASE = clkPhase & Clk_Brea1_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clk_Brea1_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Clk_Brea1_PHASE & Clk_Brea1_PHASE_MASK;
}

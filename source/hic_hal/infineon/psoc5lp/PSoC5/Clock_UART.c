...
/*******************************************************************************
* Function Name: Clock_UART_Start
********************************************************************************
*
* Summary:
...
void Clock_UART_Start(void)
{
    ...
    Clock_UART_CLKEN |= Clock_UART_CLKEN_MASK;
	Clock_UART_CLKSTBY |= Clock_UART_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clock_UART_Stop
********************************************************************************
*
* Summary:
...
void Clock_UART_Stop(void)
{
    ...
    Clock_UART_CLKEN &= (uint8)(~Clock_UART_CLKEN_MASK);
	Clock_UART_CLKSTBY &= (uint8)(~Clock_UART_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_UART_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Clock_UART_CLKEN & Clock_UART_CLKEN_MASK) != 0u)
    {
...
#else
        CLK_DIST_DMASK = Clock_UART_CLKEN_MASK;
        ...
        oldDivider = CY_GET_REG16(Clock_UART_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_UART_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_UART_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Clock_UART_SetDividerRegister
********************************************************************************
*
* Summary:
...

{
    ...
    uint8 currSrc = Clock_UART_GetSourceRegister();
    uint16 oldDivider = Clock_UART_GetDividerRegister();
    ...
    if (clkDivider != oldDivider)
    {
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_UART_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Clock_UART_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Clock_UART_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_UART_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_UART_MOD_SRC |= modeBitMask & (uint8)Clock_UART_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_UART_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_UART_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_UART_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_UART_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_UART_MOD_SRC & (uint8)(Clock_UART_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_UART_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Clock_UART_GetDividerRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_UART_MOD_SRC |= CYCLK_SSS;
        Clock_UART_MOD_SRC =
            (Clock_UART_MOD_SRC & (uint8)(~Clock_UART_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_UART_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_UART_MOD_SRC & Clock_UART_SRC_SEL_MSK;
}

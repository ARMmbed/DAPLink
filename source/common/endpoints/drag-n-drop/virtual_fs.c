...
mbr_t mbr = {
    ...
    // Bootstrap section removed because it is constant zero
    /*uint16_t signature*/
    // Signature removed since we don't need the drive to be bootable
};
...
virtual_media_t fs[] = {
    // fs setup
    // MBR is always 512 bytes, define it as such so we don't waste half a KB of RAM
    {(uint8_t *)&mbr, 512},
